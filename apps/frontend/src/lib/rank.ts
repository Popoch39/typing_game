...
export const rankTiers: Record<RankTier, {
  color: string
  bgColor: string
  borderColor: string
  ringColor: string
...
export function getRankFromLp(lp: number): RankInfo {
  ...
  for (const [tier, config] of tiers) {
    if (lp >= config.minLp) {
      ...
      const division = tierLp < 250 ? "IV" : tierLp < 500 ? "III" : tierLp < 750 ? "II" : "I"
...
export function getNextRank(current: RankInfo): string {
  ...
  const tierOrder: RankTier[] = ["Bronze", "Silver", "Gold", "Platinum", "Diamond", "Master", "Grandmaster"]
